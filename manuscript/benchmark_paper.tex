% ACML 2026 Submission — Don't Overthink It
% Double-blind — author info omitted
% Compiled: 2026-06-23
...
Table~\ref{tab:bs_ci} reports bootstrap 95\% confidence intervals for the best model in each family. The tree ensemble CI lies entirely below the MLP CI, confirming that the gap between the best tree and the best deep learning model is statistically significant.
...
\end{tabular}
\tablenote{%
  CIs computed via bootstrap resampling (1,000 iterations) of within-family RMSE values.
  Only families with $\geq$7 experiments shown.
}
\end{table*}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.58\textwidth]{figures/fig_esm_scale.pdf}
...
\end{tabular}
\vspace{4pt}
\footnotesize{RF hyperparameters held constant across all datasets.}
\end{table*}
