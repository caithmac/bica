...
To validate this attribution with a more rigorous approach, we performed a type~II ANOVA on mean RMSE aggregated by unique configuration ($N = 236$ unique model-representation combinations). This analysis confirms the same hierarchy: model family explains 77.5\% of the explainable variance ($F = 39.2$, $p < 10^{-15}$), ligand representation 16.6\% ($F = 7.8$, $p < 10^{-6}$), and protein representation 5.8\% ($F = 3.0$, $p < 10^{-3}$). Permutation-based importance testing (1,000 shuffles per factor) further confirms that all three factors are statistically significant at $p < 0.01$, with protein representation showing the smallest effect. The lower protein representation contribution relative to the RF analysis reflects the absence of interaction effects in the linear ANOVA model, but the qualitative pattern---architecture dominating, protein contributing the least---is consistent across methods.
