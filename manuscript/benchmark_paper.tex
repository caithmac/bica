...
3. Random splits are insufficient. MolXProt's random-split evaluation~\citep{cucco2026molxprot} reports RMSE = 1.69~kcal/mol ($\approx$ 1.24~p$K_d$). Under scaffold split, our best model achieves 1.007~p$K_d$. The community should adopt scaffold splits as the minimum standard, and venues should require scaffold-split baselines for DTA papers. These recommendations echo broader calls for rigor in ML for drug discovery~\citep{jimenez2020xai} and align with free energy community standards~\citep{shirts2008mbar,mey2024benchmark}.
