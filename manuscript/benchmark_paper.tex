...
This raises an uncomfortable question: has the field been over-investing in architectural complexity while under-investing in rigorous evaluation? We present the most comprehensive empirical answer to date: 335 logged experiments spanning 15 model families---linear, tree ensembles (RF, XGBoost, LightGBM), multi-layer perceptrons, CNN-based, LSTM and Mamba, transformers, graph neural networks (GCN, GAT, Graphormer), bidirectional cross-attention (BiCA, BiCA~v2/ChemCross), and PSICHIC~\citep{hao2024psichic}. After deduplication, 324 unique experiments form our leaderboard. Every experiment uses the same BindingDB\_filtered dataset, the same Bemis--Murcko scaffold split (seed 42, with multi-seed validation at 123 and 456), and the same metrics (RMSE, Pearson $R$, Spearman $\rho$). All results are logged with bootstrap 95\% confidence intervals.
...
We conducted  systematic benchmark of model architectures for scaffold-split binding affinity prediction: 15 families, 335 experiments (324 unique), multi-seed validation, bootstrap CIs. A Random Forest with ECFP4 fingerprints and 20-feature AAC outperforms every deep learning architecture tested (seed 42 test RMSE = 1.007). Across three independent scaffold partitions (seeds 42, 123, 456), RF achieves mean RMSE of 1.044 $\pm$ 0.028, confirming that the result is robust to scaffold partitioning. Model family explains 57\% of performance variance; protein representation explains 7\%. Pretrained embeddings help deep models (up to 15\%) but provide negligible benefit to trees, and scaling ESM-2 from 8M to 650M parameters yields no improvement. Cross-dataset validation on seven BALM datasets and a protein-family split confirm these findings generalize beyond a single benchmark.
