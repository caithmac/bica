#!/usr/bin/env python3
"""Phase 3 — CPU-only: ECFP4+AAC and ECFP4-only. RF + 2 MLPs. No ESM-2, no ChemBERTa."""
...
def run_mlp(X_train, y_train, X_val, y_val, X_test, y_test, hidden_dims, seed, lr=5e-4):
    ...
    val_metrics, test_metrics, train_time, epochs, test_pred = train_torch(
        ...
        lr=lr, batch_size=BATCH_SIZE, max_epochs=200, patience=15, weight_decay=0.01
    )
...
if os.path.exists(RESULTS_CSV):
    ...
    print(f"Existing: {len(existing)} fits — will skip")

MODELS = [
    # RF
    ('RF_ECFP4+AAC',   'tree', 'rf',  lambda Xtr,ytr,Xv,yv,Xte,yte,sd: run_rf(Xtr,ytr,Xv,yv,Xte,yte,sd)),
    ('RF_ECFP4-only',  'tree', 'rf',  lambda Xtr,ytr,Xv,yv,Xte,yte,sd: run_rf(Xtr,ytr,Xv,yv,Xte,yte,sd)),
    # MLP shallow
    ('MLP_shallow_ECFP4+AAC',  'mlp', 'mlp', lambda Xtr,ytr,Xv,yv,Xte,yte,sd: run_mlp(Xtr,ytr,Xv,yv,Xte,yte,[256],sd)),
    ('MLP_shallow_ECFP4-only', 'mlp', 'mlp', lambda Xtr,ytr,Xv,yv,Xte,yte,sd: run_mlp(Xtr,ytr,Xv,yv,Xte,yte,[256],sd)),
    # MLP deep
    ('MLP_deep_ECFP4+AAC',  'mlp', 'mlp', lambda Xtr,ytr,Xv,yv,Xte,yte,sd: run_mlp(Xtr,ytr,Xv,yv,Xte,yte,[512,256,128],sd)),
    ('MLP_deep_ECFP4-only', 'mlp', 'mlp', lambda Xtr,ytr,Xv,yv,Xte,yte,sd: run_mlp(Xtr,ytr,Xv,yv,Xte,yte,[512,256,128],sd)),
]
...
with open(RESULTS_CSV, 'a', newline='') as f:
    ...
    for split_type in SPLIT_TYPES:
        for seed in SEEDS:
            ...
            for model_name, family, feat_type, fn in MODELS:
                cnt += 1
                ...
                try:
                    result = fn(Xtr, y_train, Xv, y_val, Xte, y_test, seed)
                    row = {
                        'timestamp': datetime.utcnow().isoformat(),
                        'model': model_name, 'model_family': family, 'split_type': split_type,
...
print(f"\n[Phase 3] DONE. Results: {RESULTS_CSV}")
